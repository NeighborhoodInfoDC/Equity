/**************************************************************************
 ...
 Modifications: 9/23/17 LH Fixed macros that were referring to variables used in the district level analysis
		...
		02/13/26 LH Update for 2020-24 and add MD/VA jurisdiction summaries for HIT
 **************************************************************************/
...
value $county
	...
	"51041" = "Chesterfield"
	"51087" = "Henrico"
	"24SXX" = "Maryland jurisdictions"
	"51SXX" = "Virginia jurisdictions"
	;
run;
%macro pickregion(dcregion); 
	...
	** Stack county and region data **;
	data Profile_acs_region;
		set allcounty region_agg  state_agg (where=(_freq_ > 1));
		...
		if ucounty="0" and state="Maryland" then ucounty="24SXX"; 
		if ucounty="0" and state="Virginia" then ucounty="51SXX";
		...
		if ucounty="24SXX" then order=28;
		if ucounty="51SXX" then order=29; 
